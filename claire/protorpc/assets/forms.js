...
function formatJSON(value, indent) {
    ...
    if (messages) {
      $.each(messages, function(messageIndex, message) {
        ...
        if (message.enum_type) {
          populateEnum(message.enum_type, messageName);
        }
      });
    }
  }

  function populateEnum(enums, messageName) {
    if (enums) {
      $.each(enums, function(enumIndex, enumerated) {
  ...
  function populateDescriptors(file_set) {
    ...
    $.each(file_set.file, function(index, file) {
      ...
      populateEnum(file.enum_type, '.' + file['package']);
    });
  }
